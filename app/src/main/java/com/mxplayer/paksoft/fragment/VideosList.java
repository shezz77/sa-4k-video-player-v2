package com.mxplayer.paksoft.fragment;
...
import com.mxplayer.paksoft.activity.cut_video;
import com.mxplayer.paksoft.activity.lockedmedia;
import com.mxplayer.paksoft.adapter.FolderAdapter;
import com.mxplayer.paksoft.adapter.RecyclerItemClickListener;
...
import com.mxplayer.paksoft.pinlib.CustomPinActivity;
import com.mxplayer.paksoft.player.Mediaplayer;
...
//              ArrayList<VideoViewInfo> videoRows = new ArrayList<VideoViewInfo>();
                    ...
                    if (v.getId() == R.id.clickevent1) {
                        ...
                        if (videocursor.moveToPosition(position)) {//              ArrayList<VideoViewInfo> videoRows = new ArrayList<VideoViewInfo>();
                            String title = videocursor.getString(videocursor.getColumnIndexOrThrow(MediaStore.Video.Media.TITLE));
                            int video_duration = videocursor.getColumnIndexOrThrow(MediaStore.Video.Media.DURATION);
                            int video_index = videocursor.getColumnIndexOrThrow(MediaStore.Video.Media._ID);
                            if (!db.CheckIsVideoexits("track_id", videocursor.getString(video_index))) {
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        if(sharedPreferences.getInt("THEME", 0)!=111) {
            icon = searchView.findViewById(androidx.appcompat.R.id.search_button);
            icon.setColorFilter(Color.BLACK);
        ...
        if (id == R.id.sorting) {
            if(sharedPreferences.getInt("THEME", 0)!=111) {
                if (sharedPreferences.getBoolean("is_grid", false)) {
                    ...
                    item.setIcon(R.drawable.baseline_view_compact_black_24dp);
                }else{
                    item.setIcon(R.drawable.baseline_view_compact_black_24dp);
                    editor.putBoolean("is_grid",true).apply();
                }
                adapter = new VideoAdapter(getContext(), videoModelList,sharedPreferences.getBoolean("is_grid",false));
                recyclerView.setLayoutManager(sharedPreferences.getBoolean("is_grid", false) ? new StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL) :new LinearLayoutManager(getContext()) );
                recyclerView.setAdapter(adapter);
                adapterclick();
            }else{
                if (sharedPreferences.getBoolean("is_grid", false)) {
                    editor.putBoolean("is_grid",false).apply();
                    item.setIcon(R.drawable.baseline_view_comfy_white_24dp);
                }else{
                    item.setIcon(R.drawable.baseline_view_comfy_white_24dp);
                    editor.putBoolean("is_grid",true).apply();
                }
...
//                                            if (video_id == -1 || video_id  == -1 && video_id != 0) {
